      ...
      <Section bg={RG.creamSoft} pad="lg">
        <Container>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }} className="rg-hero-grid rg-sobre-areas-grid">
            <Reveal>
              <div className="rg-service-hero-img-wrap" style={{ borderRadius: 16, overflow: 'hidden', position: 'relative', background: RG.white, border: `1px solid ${RG.line}` }}>
                <img src="/assets/fotos/servico-tecarterapia-maquina.jpg?v=20260716a" alt="Consola do equipamento Winback" style={{ width: '100%', height: 'auto', display: 'block' }} />
              </div>
            </Reveal>
            <Reveal delay={80}>
              <div style={{
              ...
              {detail.equipment.badge && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 24 }}>
                  <div style={{ width: 56, height: 56, borderRadius: 10, overflow: 'hidden', flexShrink: 0, border: `1px solid ${RG.line}` }}>
                    <img src="/assets/fotos/servico-tecarterapia-winback-selo.jpg?v=20260715c" alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                  </div>
...
function ContactosPage() {
  ...
  return (<>
    ...
    {/* ── Hero 2 colunas ── */}
    <section className="rg-contact-hero" style={{ paddingTop: 130, paddingBottom: 56, background: RG.white, position: 'relative', overflow: 'hidden' }}>
      ...
      <Container>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }} className="rg-hero-grid rg-contact-hero-grid">
          <div>
          ...
          {/* Cartão de visita */}
          <Reveal delay={100}>
            <div className="rg-contact-card" style={{ background: RG.white, border: `1px solid ${RG.line}`, borderRadius: 16, padding: 32, boxShadow: '0 20px 44px -26px rgba(16,52,50,0.28)' }}>
              {[
                {
                  label: t('contactos.morada_label'), value: t('contactos.morada_val'), href: null,
                  icon: 'M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z',
                  maps: 'https://maps.google.com/maps?q=Rua+Padre+Ant%C3%B3nio+Vieira+58%2C+8100-611+Loul%C3%A9',
                },
                {
                  label: t('contactos.telefone_label'), value: '(+351) 961 899 364', href: 'tel:+351961899364',
                  icon: 'M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1C10.61 21 3 13.39 3 4c0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z',
                },
                {
                  label: t('contactos.email_label'), value: 'fisioritaguerreiro@gmail.com', href: 'mailto:fisioritaguerreiro@gmail.com',
                  icon: 'M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z',
                  cls: 'rg-contact-email',
                },
                {
                ...
                const valueStyle = { fontFamily: F_DISPLAY, fontSize: 20, fontWeight: 300, color: RG.ink, letterSpacing: '-0.02em', lineHeight: 1.35, whiteSpace: 'pre-line', overflowWrap: 'anywhere' };
                return (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '40px minmax(0, 1fr)', gap: 16, alignItems: 'start', paddingTop: i > 0 ? 24 : 0, paddingBottom: last ? 0 : 24, borderBottom: last ? 'none' : `1px solid ${RG.line}` }}>
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: RG.tealWash, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                        <path d={item.icon} fill={RG.tealDark} />
                      </svg>
                    </div>
                    <div>
                      <div style={{ fontFamily: F_BODY, fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: RG.tealDark, marginBottom: 6 }}>{item.label}</div>
                      {item.href
                        ? <a href={item.href} className={item.cls} style={{ ...valueStyle, textDecoration: 'none', display: 'block' }}>{item.value}</a>
                        : <div className={item.cls} style={valueStyle}>{item.value}</div>
                      }
                      {item.maps && (
                        <a href={item.maps} target="_blank" rel="noopener noreferrer" className="rg-maps-link" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 10, fontFamily: F_BODY, fontSize: 13, fontWeight: 700, color: RG.tealDark, textDecoration: 'none' }}>
